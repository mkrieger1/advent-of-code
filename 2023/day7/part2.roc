...
handType = \cards ->
    ...
    numJokers = counts |> Dict.get 'J' |> Result.withDefault 0
    sortedCounts =
        when counts |> Dict.remove 'J' |> Dict.values is
            [] -> [numJokers]
            countsWithoutJoker ->
                countsWithoutJoker
                |> List.sortDesc
                |> List.update 0 \c -> c + numJokers

    when sortedCounts is
        [5] -> FiveOfAKind
        [4, 1] -> FourOfAKind
        [3, 2] -> FullHouse
        [3, 1, 1] -> ThreeOfAKind
...
cardScore = \card ->
    labels = "J23456789TQKA" |> Str.toUtf8
    labels
    ...
    when (first, second) is
        ([a, .. as rest1], [b, .. as rest2]) ->
            when (cardScore a) |> Num.compare (cardScore b) is
                ...
                cmp -> cmp
        _ -> crash "Unexpected empty list in cardsCmp"
